...
// Use DeepSeek API (which is OpenAI compatible)
const getDeepSeekApiKey = () => {
  const apiKey = process.env.DEEPSEEK_API_KEY;
  
  if (apiKey && apiKey !== 'your-deepseek-api-key-here' && apiKey.length > 20) {
    return apiKey;
  }
  
  console.warn('No valid DeepSeek API key found. Please set DEEPSEEK_API_KEY environment variable.');
  return 'your-deepseek-api-key-here';
};

export class DeepSeekAI {
  private static instance: DeepSeekAI;
  private openai: OpenAI;
  
  public static getInstance(): DeepSeekAI {
    if (!DeepSeekAI.instance) {
      DeepSeekAI.instance = new DeepSeekAI();
    }
    return DeepSeekAI.instance;
  }
  ...
  private constructor() {
    ...
    if (apiKey === 'your-deepseek-api-key-here') {
      throw new Error('Invalid DeepSeek API key format - using Mock AI instead');
    }
